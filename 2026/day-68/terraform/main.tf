resource "aws_default_vpc" "aish_vpc" {
  tags = {
    Name = "default VPC"
  }
}

resource "aws_default_subnet" "aish_default_subnet" {
...
#  vpc_id = aws_default_vpc.aish_vpc.id
...
#  subnet_id      = aws_default_subnet.aish_default_subnet.id
